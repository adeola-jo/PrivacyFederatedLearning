...
def draw_conv_filters(epoch, step, layer, save_dir):
  ...
  for i in range(1):
    ...
    for j in range(num_filters):
      ...
      img_normalized = (img - np.min(img)) / (np.max(img) - np.min(img))  # Normalize to [0, 1]
      img_scaled = (img_normalized * 255).astype(np.uint8)  # Scale to [0, 255] and convert to uint8
    ...
    ski.io.imsave(os.path.join(save_dir, filename), img_scaled)
