...
#NOTE:  variance scaling initializer is used to initialize weights in a way that the variance of the outputs of each layer is approximately equal to 1. It is used to prevent the variance of the gradients from exploding or vanishing prematurely.
#NOTE:  The truncated normal distribution is also known as bounded normal distribution. It is bounded between two values a and b. The standard normal distribution is unbounded. The truncated normal distribution is used to generate random numbers from an normal distribution in a specified range. 
...
class Convolution(Layer):
  ...
  def __init__(self, input_layer, num_filters, kernel_size, name, padding='SAME',
    ...
    Args:
      ...
      num_filters: number of filters in this layer (also the number of channels in output)
    ...
    N, C, H, W = input_layer.shape # N: number of images, C: number of channels, H: height, W: width
    self.C = C
    self.N = N
    self.num_filters = num_filters # number of filters (also the number of channels in output)
    self.kernel_size = kernel_size # size of kernel
...
class MaxPooling(Layer):
  def __init__(self, input_layer, name, pool_size=2, stride=2):
    ...
    self.name = name
    self.input_shape = input_layer.shape
    N, C, H, W = self.input_shape # N: number of images, C: number of channels, H: height, W: width
    self.stride = stride
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    self.y = y
    loss = -np.sum(y * np.log(self.probs)) / x.shape[0]
    return loss
...
class L2Regularizer():
  ...
  def forward(self):
    ...
    """
    #take the L2 norm of the weights and multiply by the weight decay and divide by 2
